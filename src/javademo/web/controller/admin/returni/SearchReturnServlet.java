...
@WebServlet("/SearchReturnServlet")
public class SearchReturnServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(search.matches("\\d+")){
            try {
                returnList = returnService.searchReturnsByStuId(search);
            ...
            try {
                ...
                returnList = returnService.searchReturnsByBookId(s);//前端搜索book_id时前要加字母b
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
            listCount = returnList.size();
